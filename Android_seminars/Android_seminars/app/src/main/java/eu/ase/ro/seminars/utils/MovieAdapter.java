package eu.ase.ro.seminars.utils;
...
import eu.ase.ro.seminars.R;


public class MovieAdapter extends ArrayAdapter<Movie> {
    TextView tvTitle, tvReleaseDate, tvProfit, tvMovieGenre, tvPlatform;
    private final int resource;
    private final List<Movie> objects;
    private final LayoutInflater inflater;

    //the constructor of the custom adapter received four parameters:
    //1. the context
    //2. resource -> the id of the layout resource associated to the custom adapter
    //3. objects -> the list of objects that needs to be passed to the AdapterView via the custom Adapter
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        @SuppressLint("ViewHolder") View row = inflater.inflate(this.resource, parent, false);
